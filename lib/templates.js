...
function resourceLine(resourceUrl) {
  return resourceUrl ? `\n\nNew to Metaview or forgot how the admit prompt works? [Admit & Submit guide](${resourceUrl})` : '';
}

// Always listed one per line, in both templates - a single inlined
// sentence ("was it: a) ... b) ... c) ... or d) ...?") reads faster to
// write but slower to actually answer against.
const ABCD_OPTIONS = `a) Forgot to admit Metaview
...
${ABCD_OPTIONS}${resourceLine(resourceUrl)}
...
No worries either way, just want the data 🙏${resourceLine(resourceUrl)}`;
}
