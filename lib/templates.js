 ...
 */
function renderNudgeMessage(group) {
  const fname = require('./filters').firstName(group.interviewerName);
  if (group.misses.length === 1) {
    const m = group.misses[0];
    return singleMissMessage({ firstName: fname, eventName: m.eventName, date: formatDate(m.startTime) });
  }
  return multiMissMessage({
    firstName: fname,
    misses: group.misses.map((m) => ({ eventName: m.eventName, time: `${formatDate(m.startTime)}, ${formatTime(m.startTime)}` })),
  });
}
