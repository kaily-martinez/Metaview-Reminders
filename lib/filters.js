...
/**
 * Checks an interviewer group against config.json's `doNotMessage` list -
 * named people whose misses should still count toward the report (by-team,
 * repeat-offenders, overall miss rate) but who should never actually get a
 * nudge DM. Distinct from `excludedEventTitlePatterns`/`excludeConversationIds`
 * in filterRealMisses, which mean "not a real miss at all" (dropped from the
 * report too) - this only suppresses the message, not the miss itself.
 * Matches by email first (case-insensitive) when both sides have one, else
 * falls back to name, since a Metaview participant record isn't guaranteed
 * to carry an email.
 */
function isDoNotMessage(group, doNotMessage) {
  if (!doNotMessage || !doNotMessage.length) return false;
  const name = (group.interviewerName || '').toLowerCase();
  const email = (group.interviewerEmail || '').toLowerCase();
  return doNotMessage.some((p) => {
    const pEmail = (p.email || '').toLowerCase();
    const pName = (p.name || '').toLowerCase();
    return (pEmail && email && pEmail === email) || (pName && name && pName === name);
  });
}

/**
 * Checks an interviewer group against config.json's `excludedInterviewers`
 * list - internal recruiting/People Ops staff (recruiters, coordinators,
 * sourcers) who show up as "interviewer" mostly through scheduling/logistics
 * roles rather than as the actual panel interviewer, already know Metaview
 * needs to be admitted, and whose misses are usually a cancelled/rescheduled
 * call rather than a real compliance gap. Unlike `isDoNotMessage`, a match
 * here means the miss is dropped entirely - not messaged AND not counted
 * anywhere in the report - the same "not a real miss" treatment as
 * `excludedEventTitlePatterns`/`excludeConversationIds` in `filterRealMisses`,
 * just keyed on the person instead of the conversation.
 *
 * Matching is deliberately looser than `isDoNotMessage`'s exact name match:
 * these names were configured as whatever was on hand (a first name, an
 * initial, a nickname), not necessarily Metaview's full display name, so a
 * name match here is a case-insensitive substring check in either direction
 * ("davis" matches "Davis Momii", "Aaron F" matches "Aaron Fried") on top of
 * the same exact-email check.
 */
function isExcludedInterviewer(group, excludedInterviewers) {
  if (!excludedInterviewers || !excludedInterviewers.length) return false;
  const name = (group.interviewerName || '').toLowerCase();
  const email = (group.interviewerEmail || '').toLowerCase();
  return excludedInterviewers.some((p) => {
    const pEmail = (p.email || '').toLowerCase();
    const pName = (p.name || '').toLowerCase();
    if (pEmail && email && pEmail === email) return true;
    if (pName && name && (name === pName || name.includes(pName) || pName.includes(name))) return true;
    return false;
  });
}
