...
/**
 * Parses a Slack reply against the daily nudge's a/b/c/d template.
...
function parseReply(text) {
  ...
  // Matches a leading a/b/c/d that stands alone or is followed by a
  // separator (")", ".", ":", "-", whitespace) - e.g. "a", "a)", "a) forgot".
  // Anything else (including a word that merely starts with a/b/c/d) is
 ...
 * (the a/b/c/d code, or null) and `.replyRaw` (free text when not a clean
 * a/b/c/d match, or null when there's no reply yet).
 */
function tallyReasons(entries) {
  const counts = { a: 0, b: 0, c: 0, d: 0, other: 0, noReply: 0 };
